...
from app.models.clip import vectorize_images
...
class VectorService:
    ...
    def insert_images(self, filenames: list[str], vectors: list[list[float]]) -> None:
        metadata = [{"filename": f} for f in filenames]
        self.db.insert_many(ids=filenames, vectors=vectors, metadata=metadata)
        
    def process_images(self, filenames: list[str]) -> None:
        features = vectorize_images(filenames)
        self.insert_images(
            filenames=filenames,
